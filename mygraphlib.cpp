...
bool GenerateVertexPositions(ListDigraph &g,
			     ArcValueMap &custo,
			     DNodePosMap &posx,
			     DNodePosMap &posy)
{
  ListGraph gtmp;
  EdgeValueMap cc(gtmp);
  std::map<DNode,Node> ref;
  std::map<Node,DNode> i_ref;
  for(DNodeIt n(g); n!=INVALID; ++n) {
	  ref[n] = gtmp.addNode();
	  i_ref[ref[n]] = n;
  }
  for(ArcIt a(g); a!=INVALID; ++a) {
	  Edge e = gtmp.addEdge(ref[g.source(a)], ref[g.target(a)]);
	  cc[e] = custo[a];
  }
  NodePosMap px(gtmp);
  NodePosMap py(gtmp);
  bool r = GenerateVertexPositions(gtmp, cc, px,py);
  if ( r ){
	  for(NodeIt n(gtmp); n!=INVALID; ++n){
		  posx[i_ref[n]] = px[n];
		  posy[i_ref[n]] = py[n];
	  }
  }
...
int ViewListDigraph(ListDigraph& g,
      ...
      string text)
{
  DNodePosMap px(g); //x-position of the nodes
  DNodePosMap py(g); //y-position of the nodes
  ArcValueMap weight(g);
  for (ArcIt a(g); a!=INVALID; ++a) weight[a] = 1;
  GenerateVertexPositions(g,weight,px,py); 
  return ViewListDigraph(g, vname, aname, px, py, vcolor, acolor, text);
}
      ...
      string text) // text displayed below the figure
{
  ...
  for (ArcIt e(g); e!=INVALID; ++e) {
    if (ecolor[e]==NOCOLOR) continue;
    //fprintf(fp,"\t%s  -- %s [label = \"%s\", color=\"%s\" ];\n",vname[g.u(e)].c_str(),vname[g.v(e)].c_str(),ename[e].c_str(),ColorName(ecolor[e]).c_str());
    fprintf(fp,"\t%s -> %s [label = \"%s\", color=\"%s\" ];\n",vname[g.source(e)].c_str(),vname[g.target(e)].c_str(),aname[e].c_str(), ColorName(ecolor[e]).c_str());
  }
  fprintf(fp,"label=\"%s\";\nfontsize=50;\n",text.c_str());
  fprintf(fp,"}\n");
  fclose(fp);
